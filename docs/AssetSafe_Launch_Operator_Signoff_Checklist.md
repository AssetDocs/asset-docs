...
| Data lifecycle | PITR restore drill passed and signed off | Accepted MVP - scheduled | Michael Lewis / platform owner | `docs/AssetSafe_Backup_Restore_Runbook.md`: PITR restore drill scheduled for 2026-07-07 at 10:00 AM America/Chicago as a `pitr_to_scratch` drill from production project `leotcbfpqiekgkgumecn` to a scratch Supabase project. Limited MVP accepts scheduled drill; completed drill and `restore_drill_runs` sign-off evidence remain preferred before broad launch. `docs/AssetSafe_Data_Lifecycle_External_Controls_MVP_Signoff.md` records 7-day PITR accepted for MVP and 14-day PITR deferred to P1. | 2026-07-05 |
...
| Security | Incident tabletop completed or scheduled before broad launch | Accepted MVP - scheduled | Michael Lewis / security owner | `docs/AssetSafe_Security_Incident_Tabletop_Agenda.md`: tabletop scheduled for 2026-07-06 at 10:00 AM America/Chicago. Scenario: suspected unauthorized access or exposed storage object affecting user asset documents. Participants: Michael Lewis as incident commander/security/platform/support owner, engineering observer if available, counsel observer TBD/not assigned for MVP. Completion evidence remains preferred before broad launch. | 2026-07-05 |
...
| Security | Incident / scan readiness | Runbooks exist: `docs/AssetSafe_Security_Incident_Response_Runbook.md`, `docs/AssetSafe_Vulnerability_Scan_Runbook.md`, `docs/AssetSafe_Audit_Log_Retention_Runbook.md`; Resend webhook recovery and clean dependency audit/build evidence captured in `docs/AssetSafe_Launch_Evidence_Run_2026_07_01.md`; incident tabletop scheduled in `docs/AssetSafe_Security_Incident_Tabletop_Agenda.md` | Complete tabletop evidence before broad launch if practical; run staging ZAP/manual auth scan and triage High/Critical findings before broad launch / P1 hardening |
| Security | Launch decision packet | `docs/AssetSafe_Security_Launch_Decision_Memo.md` separates accepted MVP decisions from evidence-required items: secret manager and incident contacts are accepted; vulnerability scan is deferred as accepted MVP risk pending staging; tabletop is scheduled for MVP | Complete tabletop evidence before broad launch if practical; complete staging scan evidence before broad launch / P1 hardening |
