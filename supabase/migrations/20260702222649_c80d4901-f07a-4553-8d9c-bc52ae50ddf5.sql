...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.stripe_event_replay_requests TO authenticated;
...
DROP POLICY IF EXISTS "Admins can view stripe event replay requests" ON public.stripe_event_replay_requests;
...
BEGIN
  ...
  INSERT INTO public.stripe_event_replay_requests (
    stripe_event_id, requested_by, notes, status
  ) VALUES (
    p_stripe_event_id, p_requested_by,
    NULLIF(BTRIM(COALESCE(p_notes, '')), ''), 'requested'
  )
