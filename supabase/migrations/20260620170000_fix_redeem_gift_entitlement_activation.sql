-- Gift redemption must activate the recipient entitlement atomically.
-- The token-hardened redeem_gift RPC replaced the older claim_gift_subscription
-- path, so keep the security model while restoring subscription activation.
