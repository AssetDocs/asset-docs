-- Fix function search path security issue by properly dropping and recreating functions with triggers

-- Drop triggers that depend on the function
DROP TRIGGER IF EXISTS validate_leads_email ON public.leads;
DROP TRIGGER IF EXISTS validate_subscribers_email ON public.subscribers;
...
-- Recreate missing triggers for timestamp updates
