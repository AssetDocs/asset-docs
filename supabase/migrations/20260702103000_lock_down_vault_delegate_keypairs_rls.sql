...
-- The only direct table access allowed to authenticated users is their own keypair.
-- Owners/delegates that need another user's public key must use
-- public.get_vault_delegate_public_key(p_delegate_user_id), which exposes only public_key_jwk.
