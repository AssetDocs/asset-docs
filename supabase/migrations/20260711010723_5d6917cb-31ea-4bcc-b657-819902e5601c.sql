...
CREATE POLICY "Account members can view VIP contacts"
  ON public.vip_contacts FOR SELECT TO authenticated
  USING (auth.uid() = user_id OR public.has_account_access(auth.uid(), user_id, 'read_only'));

CREATE POLICY "Full access can create VIP contacts"
  ON public.vip_contacts FOR INSERT TO authenticated
  WITH CHECK (
...
CREATE POLICY "Full access can update VIP contacts"
  ON public.vip_contacts FOR UPDATE TO authenticated
  USING (
    public.is_owner_account_writable(user_id)
    AND (auth.uid() = user_id OR public.has_account_access(auth.uid(), user_id, 'full_access'))
  )
...
CREATE POLICY "Full access can delete VIP contacts"
  ON public.vip_contacts FOR DELETE TO authenticated
  USING (
    public.is_owner_account_writable(user_id)
    AND (auth.uid() = user_id OR public.has_account_access(auth.uid(), user_id, 'full_access'))
  );
...
CREATE POLICY "Account members can view contact attachments"
  ON public.vip_contact_attachments FOR SELECT TO authenticated
  USING (auth.uid() = user_id OR public.has_account_access(auth.uid(), user_id, 'read_only'));

CREATE POLICY "Full access can create contact attachments"
  ON public.vip_contact_attachments FOR INSERT TO authenticated
  WITH CHECK (
    public.is_owner_account_writable(user_id)
    AND (auth.uid() = user_id OR public.has_account_access(auth.uid(), user_id, 'full_access'))
  );
...
CREATE POLICY "Full access can update contact attachments"
  ON public.vip_contact_attachments FOR UPDATE TO authenticated
  USING (
    public.is_owner_account_writable(user_id)
    AND (auth.uid() = user_id OR public.has_account_access(auth.uid(), user_id, 'full_access'))
  )
...
CREATE POLICY "Full access can delete contact attachments"
  ON public.vip_contact_attachments FOR DELETE TO authenticated
  USING (
    public.is_owner_account_writable(user_id)
    AND (auth.uid() = user_id OR public.has_account_access(auth.uid(), user_id, 'full_access'))
  );
