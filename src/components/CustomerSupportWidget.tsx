...
const CustomerSupportWidget: React.FC = () => {
  ...
  const getSystemContext = () => {
    const path = location.pathname;
    let ctx = "You are a helpful customer support assistant for Asset Safe, a property documentation and asset management application.";
    if (path.includes('/photo-upload') || path.includes('/photos')) {
      ctx += " The user is on the photo upload/management section.";
    } else if (path.includes('/properties')) {
      ctx += " The user is on the property management section.";
    } else if (path.includes('/pricing')) {
      ctx += " The user is viewing pricing information.";
    } else if (path.includes('/account')) {
      ctx += " The user is in account settings.";
    }
    ctx += " Keep responses concise, helpful, and focused on Asset Safe features. If you don't know something, suggest contacting support.";
    return ctx;
  };
  ...
  const generateContextualResponse = async (userMessage: string): Promise<string> => {
    ...
    if (!session) {
      return "Please log in to use the AI support assistant, or contact our support team directly for help.";
    }
    ...
    try {
      ...
      return "I'm having trouble connecting right now. Please try again in a moment or contact support for help.";
    }
  };
