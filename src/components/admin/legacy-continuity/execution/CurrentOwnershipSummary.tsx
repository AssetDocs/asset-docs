...
const CurrentOwnershipSummary: React.FC<{ accountId: string }> = ({ accountId }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: legacy } = await supabase.from('legacy_admins').select('legacy_admin_user_id,status').eq('account_id', accountId).eq('status', 'active').maybeSingle();
      setData({ account: acc, profile, memberCount, legacy });
    })();
  }, [accountId]);

  if (!data) return <div className="text-sm text-muted-foreground p-4">Loading account…</div>;
  const { account, profile, memberCount, legacy } = data;
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-1">
        ...
        <Row label="Legacy Admin" value={legacy ? `${legacy.legacy_admin_user_id?.slice(0,8)}…` : 'None designated'} />
